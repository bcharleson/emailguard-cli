import type { EmailGuardClient as IEmailGuardClient } from './types.js';
import {
  AuthError,
  NotFoundError,
  ValidationError,
...
export class EmailGuardClient implements IEmailGuardClient {
  ...
  async request<T>(options: {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (error instanceof TypeError && String(error.message).includes('fetch')) {
          throw new EmailGuardError(`Network error: ${error.message}`, 'NETWORK_ERROR');
        }
    ...
    throw lastError ?? new EmailGuardError('Request failed after retries', 'MAX_RETRIES');
  }
